...
export function MealCell({
  ...
  if (recipes.length === 0) {
    return (
      <button
        onClick={onAdd}
        className="w-full h-[100px] bg-gray-50/50 hover:bg-emerald-50 rounded-lg transition-all group flex flex-col items-center justify-center gap-1.5 border border-gray-200/40 hover:border-emerald-300"
      >
    ...
    <div className="w-full min-h-[100px] space-y-1.5">
      {recipes.map((recipe, index) => {
        ...
        return (
          <div
            key={recipe.id}
            className="relative bg-gradient-to-br from-emerald-500 to-teal-500 rounded-lg p-2.5 hover:shadow-lg transition-all cursor-pointer group"
